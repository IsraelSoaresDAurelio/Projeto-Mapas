...
class PassoBuscar(ctk.CTkFrame):
    def __init__(self, master, app):
        ...
        # (compacto: só o essencial, pouco espaço em branco)
        cartao_busca = Cartao(self)
        cartao_busca.grid(row=0, column=0, sticky="ew", pady=(0, 14))
        ...
        interno.grid(row=0, column=0, columnspan=2, sticky="ew", padx=20, pady=18)
        ...
        self.campo_pasta_busca.grid(row=0, column=0, sticky="ew", pady=(0, 14))
        ...
        ).grid(row=2, column=0, sticky="ew", pady=(3, 8))

        self.entrada_termos = ctk.CTkEntry(
            interno, height=42, font=FONTE_BASE, corner_radius=9,
            border_color=COR_BORDA, fg_color="#FBFCFE",
            placeholder_text="ex.: zona 1 zona 2 zona 3",
        )
        self.entrada_termos.grid(row=3, column=0, sticky="ew", pady=(0, 14))

        # "Para onde copiar" fica logo abaixo dos campos de busca, no mesmo
        # cartão, bem perto de onde o usuário acabou de preencher.
        ...
        self.campo_pasta_destino.grid(row=4, column=0, sticky="ew", pady=(0, 6))

        self.botao_limpar_destino = ctk.CTkButton(
            interno, text="Limpar pasta de destino", height=34, corner_radius=9,
        ...
        self.botao_limpar_destino.grid(row=5, column=0, sticky="w", pady=(0, 14))

        linha_botoes = ctk.CTkFrame(interno, fg_color="transparent")
        linha_botoes.grid(row=6, column=0, sticky="ew")
        linha_botoes.grid_columnconfigure(0, weight=1)
        linha_botoes.grid_columnconfigure(1, weight=1)

        self.botao_buscar = ctk.CTkButton(
            linha_botoes, text="Buscar versões mais recentes", height=44, corner_radius=9,
        ...
        self.botao_salvar = ctk.CTkButton(
            linha_botoes, text="Salvar arquivos na pasta destino", height=44, corner_radius=9,
        ...
        ctk.CTkLabel(
            ...
            anchor="w", fg_color=COR_STATUS_FUNDO, corner_radius=8, height=32,
        ).grid(row=7, column=0, sticky="ew", pady=(10, 0), ipady=4)

        # -- Cartão 2: resultado da busca -------------------------------------
        cartao_resultado = Cartao(self)
        cartao_resultado.grid(row=1, column=0, sticky="nsew")
        ...
        envoltorio_tabela.grid(row=1, column=0, sticky="nsew", padx=20, pady=(0, 20))
...
class PassoPdf(ctk.CTkFrame):
    def __init__(self, master, app):
        ...
        cartao_pasta.grid(row=0, column=0, sticky="ew", pady=(0, 14))
        cartao_pasta.grid_columnconfigure(0, weight=1)

        interno = ctk.CTkFrame(cartao_pasta, fg_color="transparent")
        interno.grid(row=0, column=0, sticky="ew", padx=20, pady=18)
        interno.grid_columnconfigure(0, weight=1)
        ...
        self.campo_pasta_pdf.grid(row=0, column=0, sticky="ew", pady=(0, 14))
        ...
        self.botao_gerar = ctk.CTkButton(
            linha_botoes, text="Gerar PDF único", height=44, corner_radius=9,
            fg_color=COR_VERDE, hover_color=COR_VERDE_HOVER, font=FONTE_BOTAO,
        ...
        self.botao_abrir = ctk.CTkButton(
            linha_botoes, text="Abrir no navegador", height=44, corner_radius=9,
            fg_color=COR_NAVY_BOTAO, hover_color=COR_NAVY_BOTAO_HOVER, font=FONTE_BOTAO,
        ...
        self.botao_imprimir = ctk.CTkButton(
            linha_botoes, text="Imprimir", height=44, corner_radius=9,
            fg_color=COR_CINZA_BOTAO, hover_color=COR_CINZA_BOTAO_HOVER,
        ...
        ctk.CTkLabel(
            interno, textvariable=self.aviso_var, font=FONTE_AJUDA, text_color=COR_TEXTO_SECUNDARIO,
            anchor="w", fg_color=COR_STATUS_FUNDO, corner_radius=8, height=32,
        ).grid(row=2, column=0, sticky="ew", pady=(10, 0), ipady=4)
        ...
        cabecalho.grid(row=0, column=0, sticky="ew", padx=20, pady=(16, 8))
...
class AplicacaoMapas(ctk.CTk):
    ...
    def _definir_icone(self):
        ...
        # o customtkinter reseta o ícone da janela pouco depois de abrir
        # (troca de volta para o ícone padrão do Tk); reaplicamos várias
        # vezes nos primeiros instantes para garantir que o ícone
        # personalizado realmente fique, mesmo em máquinas mais lentas.
        aplicar()
        for atraso_ms in (50, 150, 300, 600, 1200):
            self.after(atraso_ms, aplicar)
    ...
    def abrir_ajuda(self):
        ...
        aplicar_icone()
        janela.after(250, aplicar_icone)
